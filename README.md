...
**穴のまわりが線で囲まれているか**、**丸い形にすべて中心線が交わっているか**、
**断面図の外形が側面図にも現れるか**などを検算する
...
問題は `tools/make_problems.py` に**部品の形として一度だけ**定義する。直方体・角柱・
円柱・穴で宣言すれば、**正面図・側面図・断面図はすべてそこから機械的に導かれる**。
座標を手で並べてはいけない（同じ部品を 3 回描くことになり、中心線や見え掛かり線の
描き漏れ・3 面の食い違いが必ず起きる）。`src/Problems.gs` は生成物なので直接編集しない。

```python
part = Part(w=12, h=12, d=8)
...
│   ├── solid.py              # 部品の形の宣言と、正面図・側面図・断面図の導出
